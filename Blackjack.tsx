...
class BlackjackGame {
  ...
  startGame(bet: number, onGameStart: (playerHand: Hand, dealerHand: Hand) => void): void {
    if (!this.canStartGame(bet) || reachedLimit()) {
      return;
    }

    win(-bet);

    const playerHand = new Hand([this.blackjackLogic.safeDrawCard(), this.blackjackLogic.safeDrawCard()]);
    const dealerHand = new Hand([this.blackjackLogic.safeDrawCard(), this.blackjackLogic.safeDrawCard()]);

    onGameStart(playerHand, dealerHand);
  }
...
class BlackjackUI extends React.Component<Record<string, never>, State> {
  ...
  constructor(props: Record<string, never>) {
    ...
    this.blackjackGame = new BlackjackGame(DECK_COUNT);
  ...
  canStartGame(): boolean {
    return this.blackjackGame.canStartGame(this.state.bet);
  }
  ...
  handleGameStart(playerHand: Hand, dealerHand: Hand): void {
    ...
    if (this.blackjackLogic.getTrueHandValue(playerHand) === 21) {
      if (this.blackjackLogic.getTrueHandValue(dealerHand) === 21) {
        this.blackjackGame.finishGame(Result.Tie, this.state, this.handleGameFinish.bind(this));
      } else {
        this.blackjackGame.finishGame(Result.PlayerWonByBlackjack, this.state, this.handleGameFinish.bind(this));
      }
    } else if (this.blackjackLogic.getTrueHandValue(dealerHand) === 21) {
      this.blackjackGame.finishGame(Result.DealerWon, this.state, this.handleGameFinish.bind(this));
    }
  }
  ...
  handleGameFinish(gains: number, result: Result): void {
    this.setState({
      gameInProgress: false,
      result,
      gains: this.state.gains + gains - this.state.bet,
    });
  }

  startGame(): void {
    if (!this.blackjackGame.canStartGame() || reachedLimit()) {
      return;
    }

    this.blackjackGame.startGame(this.state.bet, this.handleGameStart.bind(this));
  }
  ...
  render(): React.ReactNode {
    ...
    const playerHandValues = this.blackjackLogic.getHandDisplayValues(playerHand);
    const dealerHandValues = this.blackjackLogic.getHandDisplayValues(dealerHand);
